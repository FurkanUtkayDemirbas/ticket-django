...
class KayitForm(UserCreationForm):
    muhatap_firmalar = forms.ModelMultipleChoiceField(
        queryset=muhatap.objects.all(),
        required=False,
        label="Firmanız (İsteğe Bağlı Birden Fazla Seçebilirsiniz)",
    )
...
class AdminKullaniciForm(forms.Form):
    ...
    muhatap_firmalar = forms.ModelMultipleChoiceField(
        ...
        required=False,
        label='Firmalar',
        widget=forms.SelectMultiple(attrs={'class': WIDGET_CLASS, 'id': 'id_muhatap_firmalar', 'data-no-ts': 'true'})
    )
    danisman_profiller = forms.ModelMultipleChoiceField(
        queryset=danisman.objects.all(),
        required=False,
        label='Danışman Profilleri',
        widget=forms.SelectMultiple(attrs={'class': WIDGET_CLASS, 'id': 'id_danisman_profiller', 'data-no-ts': 'true'})
    )
    ...
    def clean(self):
        ...
        if role == 'Firma' and not cleaned_data.get('muhatap_firmalar'):
            self.add_error('muhatap_firmalar', 'Firma rolü için en az bir firma seçmelisiniz.')

        if role == 'Danisman' and not cleaned_data.get('danisman_profiller'):
            self.add_error('danisman_profiller', 'Danışman rolü için en az bir danışman profili seçmelisiniz.')

        return cleaned_data


class AdminKullaniciDuzenleForm(forms.Form):
    """Mevcut kullanıcıyı düzenlemek için form (şifre alanı yok)."""
    ROLE_CHOICES = [
        ('Admin', 'Admin'),
        ('Danisman', 'Danışman'),
        ('Firma', 'Firma (Müşteri)'),
    ...
    def __init__(self, *args, **kwargs):
        self.kullanici = kwargs.pop('kullanici', None)
    ...
    def clean_username(self):
        ...
        if self.kullanici:
            qs = qs.exclude(pk=self.kullanici.pk)
        if qs.exists():
            raise forms.ValidationError('Bu kullanıcı adı zaten alınmış.')
        return username
